<template>
  <div class="playlist-body" :style="{ boxShadow: isVisible ? 'none' : '4px 6px gray, 8px 12px dimgray' }">
    <div class="input-body" :title="props.data.name" @click="onToggle">
    ...
    <div class="icons">
      ...
      <template v-if="props.data.name !== fallbackPlaylistName">
        ...
        <svg @click="onDeleteClick" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="red" class="bi bi-trash3" viewBox="0 0 16 16">
...
watch(isVisible, async (value) => {
  value && await _getTracksFromPlaylist();
});
...
const onDeleteClick = async () => {
  const message = `Remove ${props.data.name}?`;
  if (!confirm(message)) {
    return;
  }
...
.playlist-body {
  ...
  border-style: solid;
  border-width: 1px;
  background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('../assets/backgrounds/discs.jpg');
  background-repeat: no-repeat;
  transition: margin 150ms ease-out, top 0.35s ease;
  z-index: 0;
}
